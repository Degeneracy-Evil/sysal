...
namespace
{
    ...
    [[nodiscard]] sysal::RawStore make_test_store()
    {
        ...
        store.records.push_back(sysal::RawRecord{sysal::RawSource::ProcMemInfo, "/proc/meminfo",
                                                 "MemTotal:       32768000 kB", sysal::CollectStatus::Success, now});
    ...
    [[nodiscard]] bool record_equal(const sysal::RawRecord &a, const sysal::RawRecord &b)
    {
        ...
        auto a_ms = std::chrono::duration_cast<std::chrono::milliseconds>(a.collected_at.time_since_epoch()).count();
        auto b_ms = std::chrono::duration_cast<std::chrono::milliseconds>(b.collected_at.time_since_epoch()).count();
        return a_ms == b_ms;
    }
