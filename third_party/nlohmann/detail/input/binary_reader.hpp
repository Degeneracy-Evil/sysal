    ...
    */
    bool get_ubjson_size_value(std::size_t& result, bool& is_ndarray, char_int_type prefix = 0)
    {
        ...
        case '[':
        {
            ...
            if(is_ndarray) // ndarray dimensional vector can only contain integers, and can not
                           // embed another array
            {
                return sax->parse_error(
                    chars_read, get_token_string(),
                    parse_error::create(
                        113, chars_read,
                        exception_message(input_format, "ndarray dimentional vector is not allowed",
                                          "size"),
                        nullptr));
            }
            ...
            {
                for(auto i : dim) // test if any dimension in an ndarray is 0, if so, return a 1D
                                  // empty container
                {
                    if(i == 0)
                    {
        ...
        {
            message = "expected length type specification (U, i, u, I, m, l, M, L) after '#'; last "
                      "byte: 0x" +
                      last_token;
        }
        return sax->parse_error(
