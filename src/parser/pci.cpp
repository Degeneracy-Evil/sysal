...
namespace sysal::detail
{
    ...
    namespace
    {
        ...
        std::string normalize_lspci_address(std::string_view addr_raw)
        {
            ...
            auto second_colon =
                (first_colon != std::string_view::npos) ? addr_raw.find(':', first_colon + 1) : std::string_view::npos;
        ...
        std::optional<std::pair<std::string, std::string>> parse_lspci_line(std::string_view line)
        {
            ...
            if(!parse_hex(inner.substr(0, colon)).has_value() || !parse_hex(inner.substr(colon + 1)).has_value())
            {
                return std::nullopt;
            }
