...
std::vector<CpuInfoEntry> parse_cpuinfo(std::string_view payload,
                                        std::vector<std::string>& warnings)
{
    ...
    for(const auto& line : lines)
    {
        ...
        if(key == "processor")
        {
            ...
            {
                warnings.push_back("parse_cpu: /proc/cpuinfo processor 解析失败: " +
                                   std::string(value));
            }
        }
...
build_numa_mapping(const RawStore& raw, [[maybe_unused]] std::vector<std::string>& warnings)
{
...
std::optional<Cpu> parse_cpu(const RawStore& raw, std::vector<std::string>& warnings)
{
    ...
    if(!entries.empty())
    {
        ...
        if(vendor.find("GenuineIntel") != std::string::npos ||
           vendor.find("AuthenticAMD") != std::string::npos)
        {
